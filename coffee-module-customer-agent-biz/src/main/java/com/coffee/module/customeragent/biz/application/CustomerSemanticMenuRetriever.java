...
/** 菜单向量索引：向量持久化到 MySQL，门店菜单变更后按文本指纹自动增量更新。 */
@Component
class CustomerSemanticMenuRetriever {
    ...
    Map<String,Double> retrieve(Long storeId,String query,List<MenuItemDTO> products) {
        ...
        try {
            ...
            if(!stale.isEmpty()) {
                List<String> refresh=stale.stream().map(texts::get).toList(); Optional<List<List<Double>>> vectors=embeddings.embed(refresh);
                if(vectors.isPresent()) for(int i=0;i<stale.size();i++){int at=stale.get(i);jdbc.update("INSERT INTO agent_menu_embedding(store_id,product_code,text_hash,vector_json,updated_at) VALUES (?,?,?,?,?) ON DUPLICATE KEY UPDATE text_hash=VALUES(text_hash),vector_json=VALUES(vector_json),updated_at=VALUES(updated_at)",storeId,products.get(at).getCode(),hashes.get(at),json.writeValueAsString(vectors.get().get(i)),LocalDateTime.now());}
                cached=rows(storeId);
            }
