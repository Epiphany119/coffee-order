...
public class MenuKnowledgeBootstrapService {
    private static final Logger log = LoggerFactory.getLogger(MenuKnowledgeBootstrapService.class);
    ...
    @Async
    public CompletableFuture<Integer> bootstrapAsync(Long storeId) {
        log.info("Starting async menu knowledge bootstrap for store: {}", storeId);
        int count = doBootstrap(storeId);
        log.info("Async menu knowledge bootstrap completed for store: {}, total changed: {}", storeId, count);
        return CompletableFuture.completedFuture(count);
    }

    public int bootstrap(Long storeId) {
        return doBootstrap(storeId);
    }

    private int doBootstrap(Long storeId) {
        String storeName = jdbc.queryForObject("SELECT name FROM store WHERE id = ?", String.class, storeId);
        final String resolvedStoreName = storeName == null ? "未知门店" : storeName;
        ...
        List<AgentKnowledgeService.KnowledgeDocument> documents = rows.stream()
                .flatMap(row -> documentsFor(resolvedStoreName, row).stream()).toList();
        ...
        List<AgentKnowledgeService.KnowledgeDocument> changed = documents.stream()
                ...
                .toList();
        if (changed.isEmpty()) {
            log.info("No menu knowledge changes for store: {}", storeId);
            return 0;
        }
        log.info("Found {} knowledge documents to update for store: {}", changed.size(), storeId);
        for (int start = 0; start < changed.size(); start += 64) {
            List<AgentKnowledgeService.KnowledgeDocument> batch = changed.subList(start, Math.min(start + 64, changed.size()));
            try {
                knowledge.upsertBatch(storeId, batch);
                log.info("Processed batch {}-{} of {} for store {}", start, start + batch.size(), changed.size(), storeId);
            } catch (Exception e) {
                log.error("Failed to process batch for store {}: {}", storeId, e.getMessage(), e);
            }
        }
        return changed.size();
    }
    ...
    private List<AgentKnowledgeService.KnowledgeDocument> documentsFor(String storeName, Map<String, Object> row) {
        ...
        String code = safe(row.get("code"));
        String category = safe(row.get("category_code"));
        ...
        List<AgentKnowledgeService.KnowledgeDocument> docs = new ArrayList<>(List.of(
                ...
                new AgentKnowledgeService.KnowledgeDocument("优惠核验｜" + name,
                        String.format("推荐%s时，优惠券、会员折扣和秒杀是否可叠加必须由当前登录用户的有效券、活动状态及下单结算规则实时核验。没有真实可用优惠时，不得承诺折扣；展示金额仅作预估，确认下单时由服务端复核。",
                                name), "menu-planning-bootstrap"),
                new AgentKnowledgeService.KnowledgeDocument("排除规则｜" + name,
                        String.format("点单排除规则：当用户明确表示「不要%s」「不是%s」「不含%s」或排除%s所属品类时，%s不得出现在推荐方案中。即使语义相似度高，只要在排除列表里就必须过滤。",
                                name, name, name, category, name), "menu-exclusion-bootstrap")));
        return docs;
    }
