...
public class GrowthAgentController {
    ...
    @PostMapping("/analyze") public Result<Map<String,Object>> analyze(@PathVariable Long merchantId,@RequestBody AgentQuestion req){if(req==null||req.message==null||req.message.isBlank())throw new ServiceException(400,"请输入要分析的问题");StoreResponse s=store(merchantId);Map<String,Object> out=new LinkedHashMap<>(growthAgentService.analyze(s.getStoreId(),req.message));out.put("storeId",s.getStoreId());out.put("storeName",s.getName());return Result.success(out);}
