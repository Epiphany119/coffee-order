    ...
    <section class="workspace">
      ...
      <div class="analysis-panel panel">
        <template v-if="analysis">
          ...
          <div class="tool-line">已调用：{{ analysis.dataSources?.join(' · ') || '订单指标工具 · 履约队列工具 · 库存工具' }}</div>
          <div v-if="analysis.executionPlan?.length" class="execution-trace">
            <div class="trace-heading"><b>Agent 执行轨迹</b><span>{{ analysis.planningMode || 'policy-guarded' }}</span></div>
            <ol class="plan-list">
              <li v-for="step in analysis.executionPlan" :key="step">{{ step }}</li>
            </ol>
            <div v-if="analysis.toolCalls?.length" class="tool-call-list">
              <div v-for="tool in analysis.toolCalls" :key="tool.name" class="tool-call">
                <span class="tool-check">✓</span>
                <div><b>{{ tool.name }}</b><small>{{ tool.note }}</small></div>
                <span class="tool-meta">{{ tool.latencyMs }}ms · {{ tool.resultCount }} 条</span>
              </div>
            </div>
          </div>
        </template>
