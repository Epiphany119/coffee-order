...
public class FlashSaleApplicationService implements FlashSaleService {
    ...
    public Map<String, Object> claim(Long activityId, Long userId, String guestId) {
        ...
        if (redisEnabled) {
            ...
            // Redis 不参与本地事务：数据库事务回滚（包括通知写入失败）时统一补偿，
            // 避免只处理“库存不足/重复领取”而遗漏其他异常路径。
            TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
                @Override
                public void afterCompletion(int status) {
                    if (status != STATUS_COMMITTED) compensateRedisClaim(activityId, identity);
                }
            });
        }
        try {
    ...
    public void expireClaimsAndCloseFinishedActivities() {
        ...
        for (Map<String, Object> claim : expired) {
            ...
            // MySQL 对 JOIN UPDATE 的 affected-rows 可能按受影响表返回 1 或 2，
            // 只要状态 CAS 成功就必须补偿 Redis，不能因返回值为 2 留下脏库存。
            if (changed > 0 && redisEnabled) {
    ...
    private void requireIdentity(Long userId, String guestId) {
        ...
        if (hasUser == hasGuest) throw new ServiceException(400, "请提供一种有效的用户或游客身份");
    }
}
